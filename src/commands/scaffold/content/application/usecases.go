...
func GenerateDTO(config structs.FeatureConfiguration) string {
...
/// <summary>
/// Represents a data object for creating a %s
/// </summary>
public class Create%s
{
        %s
    ...
    `, config.EntityName, config.EntityName, fields, config.EntityName, config.EntityName, convertFields)

	return template
}

func GenerateUseCase(config structs.FeatureConfiguration) string {
	tmpl := `
